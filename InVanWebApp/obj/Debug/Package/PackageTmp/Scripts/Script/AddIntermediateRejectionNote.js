...
function SelectedIndexChangedIssueBy(id) {  
    ...
    if (SelectedIssueByNameVal != '' || SelectedIssueByNameVal != "") {
        $('#IssueByErrMsg').text('');
        document.getElementById('IssueByErrMsg').setAttribute('style', 'color:#000;');
        $('#IssueByErrMsg').hide;
...
function SaveBtnClick() {
    ...
    if (IssueByName == '---Select---') {
        ...
        document.getElementById('IssueByErrMsg').setAttribute('style', 'color:red;');
        $('#divIssueBy').focus();
        document.getElementById('divIssueBy').focus();
    ...
    var IssueByNameVal = $("#divIssueBy .option.selected").attr('data-value');
    if (IssueByNameVal == '' || IssueByNameVal == "") {
        $('#IssueByErrMsg').text('Select issue by!');
        document.getElementById('IssueByErrMsg').setAttribute('style', 'color:red;');
        $('#divIssueBy').focus();
        document.getElementById('divIssueBy').focus();
    ...
    if (tableLength > 1) {
        while (i < tableLength - 1) {
            ...
            var RejectedQty = document.getElementById("txtRejectedQty_" + i).value; //Change this with Rejected qty 
            RejectedQty = parseFloat(RejectedQty);

            $('#spanRejectedQty_' + i).text('');
            if (RejectedQty == 0 || RejectedQty == '') {
                $('#spanRejectedQty_' + i).text('You can not enter rejected quantity value zero or null!');
                document.getElementById('spanRejectedQty_' + i).setAttribute('style', 'color:red;');
            ...
            for (var j = 0; j < result.length; j++) {
                ...
                for (var i = 0; i < cellCount; i++) {
                    ...
                    if (i == 0) {
                        ...
                        t8.id = "spanRejectedQty_" + j; 
                        cell.appendChild(t8);
                    }
                    else if (i == 9) {
                        var t9 = document.createElement("input");
                        t9.id = "txtFinalStock_" + j;
...
function OnChangeQty(value, id) {
    ...
    var RejectedQty = document.getElementById("txtRejectedQty_" + rowNo).value;
    if (RejectedQty == 0 || RejectedQty == '') {
        $('#spanRejectedQty_' + rowNo).text('You can not enter rejected quantity value zero or null!');
        document.getElementById('spanRejectedQty_' + rowNo).setAttribute('style', 'color:red;');
        document.getElementById("txtRejectedQty_" + rowNo).focus();
        $('#btnSave').prop('disabled', true);
        event.preventDefault();
        return;
    }
    ...
    if (RejectedQty > avalQty) {
        $('#spanRejectedQty_' + rowNo).text('You can not issue more than available quantity!');
        document.getElementById('spanRejectedQty_' + rowNo).setAttribute('style', 'color:red;');
...
function createJson() {
    ...
    while (i < rowCount - 1) {
        ...
        var RejectedQty = (document.getElementById("txtRejectedQty_" + i)).value;        
