...
function createCustomDropdown_ProductionIndentId() {
    ...
    $('#divProductionIndentId.dropdown-select ul').before('<div class="dd-search"><input id="txtSearchValueProductionIndentId" autocomplete="off" onkeyup="filterProductionIndentId()" class="dd-searchbox" type="text" placeholder="Search for list" ><br />&nbsp;<span id="faSearch"><i class="fas fa-search"></i></span></div>');
}
function filterProductionIndentId() {
    var valThis = $('#txtSearchValueProductionIndentId').val();
    $('.dropdown-select ul > li').each(function () {
        var text = $(this).text();
        (text.toLowerCase().indexOf(valThis.toLowerCase()) > -1) ? $(this).show() : $(this).hide();
    });
